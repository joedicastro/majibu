    ...
    $('div.comment-form-container').each(function() {
        ...
        if ($form.length) {
            ...
            $cancel.click(function(event) {
                if (confirm("Vas a perder todos los cambios realizados en este comentario. ¿Seguro que quieres cancerlarlo?")){
                    if (comment_in_form) {
...
function yourWorkWillBeLost(e) {
    if(browserTester('chrome')) {
        return "¿Estás seguro de querer abandonar? Se perderan los cambios.";
    } else if(browserTester('safari')) {
        return "¿Estás seguro de querer abandonar? Se perderan los cambios.";
    } else {
        if(!e) e = window.event;
        e.cancelBubble = true;
        e.returnValue = 'Si abandonas la página, se perderan los cambios.';
